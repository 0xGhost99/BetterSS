<div align="center">

# 🔵 BetterSS
### Plugin ScreenShare Professionale per Server PvP Minecraft

[![Version](https://img.shields.io/badge/version-1.0.0-blue.svg)](https://github.com/0xGhost99/BetterSS/releases)
[![Paper](https://img.shields.io/badge/Paper-1.19+-green.svg)](https://papermc.io)
[![Java](https://img.shields.io/badge/Java-17+-orange.svg)](https://adoptium.net)
[![License](https://img.shields.io/badge/license-MIT-red.svg)](LICENSE)

*La versione gratuita del plugin ScreenShare professionale per server PvP*

> 💎 **BetterSS+** — La versione Premium è in arrivo! **COMING SOON** 🔜

</div>

---

## 📌 Descrizione

**BetterSS** è la versione **gratuita** del sistema ScreenShare per server PvP Minecraft 1.19+.
Fornisce tutte le funzionalità base per gestire le sessioni SS in modo professionale.

Vuoi funzionalità avanzate come GUI, Staff Mode, Vanish, AntiCheat integrato e molto altro?
👉 **BetterSS+ è in arrivo! Stay tuned.** 🔜

---

## ✨ Funzionalità Free

- 🌍 Mondo dedicato `betterss` automatico
- 🧊 Freeze avanzato
- 🚫 Anti-relog con ban automatico
- 📋 Scoreboard live
- 💬 Messaggi automatici
- ⏱️ Timer massimo SS
- 📁 Log YAML
- 🔔 Webhook Discord
- ⚙️ Config personalizzabile

---

## 💎 BetterSS+ — COMING SOON 🔜

> ⚠️ **La versione Premium è attualmente in sviluppo.**
> Seguici per essere notificato al lancio!

| Funzionalità | Free 🔵 | Premium 💎 |
|---|---|---|
| Mondo SS dedicato | ✅ | ✅ |
| Freeze avanzato | ✅ | ✅ |
| Anti-relog | ✅ | ✅ |
| Scoreboard live | ✅ | ✅ |
| Webhook Discord | ✅ | ✅ |
| Log YAML | ✅ | ✅ |
| Gestione ban avanzata (durata, history) | ❌ | 🔜 |
| Scelta mondo SS personalizzata | ❌ | 🔜 |
| GUI inventario gestione SS | ❌ | 🔜 |
| Report automatico per sessione | ❌ | 🔜 |
| Staff Mode (OpenInv, Vanish...) | ❌ | 🔜 |
| Vanish integrato | ❌ | 🔜 |
| AntiCheat con /ss alert | ❌ | 🔜 |
| Tasto rapido Spect / SS da alert | ❌ | 🔜 |
| Hotbar staff configurabile | ❌ | 🔜 |

---

## 📋 Requisiti

| Requisito | Versione |
|-----------|----------|
| Server | Paper 1.19+ |
| Java | 17+ |

---

## 🚀 Installazione

1. Scarica l'ultima versione da [Releases](https://github.com/0xGhost99/BetterSS/releases)
2. Metti `BetterSS.jar` nella cartella `plugins/`
3. Riavvia il server
4. Configura `plugins/BetterSS/config.yml`

---

## 🎮 Comandi

| Comando | Descrizione | Permesso |
|---------|-------------|----------|
| `/ss <player>` | Avvia SS | `betterss.use` |
| `/ss freeze <player>` | Freeza | `betterss.admin` |
| `/ss unfreeze <player>` | Defeeza | `betterss.admin` |
| `/ss clean <player>` | Termina SS | `betterss.admin` |
| `/ss ban <player>` | Banna | `betterss.ban` |
| `/ss discord <player>` | Invia Discord | `betterss.admin` |
| `/ss status <player>` | Stato SS | `betterss.admin` |
| `/ss credits` | Credits | tutti |

---

## 👤 Autore

**0xGhost99** — [@0xGhost99](https://github.com/0xGhost99)

<div align="center">

Made with ❤️ by 0xGhost99

⭐ Lascia una stella se ti piace il progetto!

</div>
